...
export default function CheckoutPage() {
  ...
  try {
    ...
    if (!res.ok) {
      throw new Error(json?.error ?? text ?? `Checkout failed (${res.status})`);
    }
      ...
      <div className="mx-auto max-w-screen-2xl px-6 py-24">
        ...
        {loadingUser ? (
          ...
          <div className="mt-8 rounded-2xl border border-border bg-card p-5">
            ...
            <Link
              className="mt-4 inline-flex rounded-md bg-lime-300 px-4 py-2 text-sm font-semibold text-black hover:bg-lime-200 transition"
              href="/login"
            >
              Login
            </Link>
